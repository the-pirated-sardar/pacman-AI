...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for item in self.items():
            weight.append(item[1] / totalValue)
            sequence.append(item[0])

        for i, value in enumerate(sequence):
            if probablity < weight[i]:
                return value
            else:
                probablity -= weight[i]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.allPositions:
            ...
            for newPosition in newPosDist.keys():
                newBeliefs[newPosition] += newPosDist[newPosition] * self.beliefs[oldPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.particles:
            newBeliefs[position] += self.getObservationProb(observation, pacmanPosition, position, jailPosition)
...
class JointParticleFilter(ParticleFilter):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                prevGhostPosition = oldParticle
                newPosDist = self.getPositionDistribution(gameState, prevGhostPosition, i, self.ghostAgents[i])
