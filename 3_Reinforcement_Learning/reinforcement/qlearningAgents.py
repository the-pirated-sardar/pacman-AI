...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        leagalActions = self.getLegalActions(state)
        actions = [] # list of actions

        #check terminal state
        if len(leagalActions) == 0:
            return None
        ...
        for action in leagalActions:
            ...
            #return a random action if there are multiple actions with the same qvalue
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for key in feature:
            self.weights[key] += self.alpha * difference * feature[key]
